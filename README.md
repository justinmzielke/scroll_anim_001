...
- **Transparent background**: No visible background or scrollbar
- **Optimized images**: 100 JPEG frames (~30MB total) for fast loading

## How It Works

`scrollviewer.html` - The main viewer
- Loads and preloads all 100 optimized image frames
- Responds to scroll input with physics-based momentum
- Fixed 1080x1920 viewport (portrait orientation)
- Hidden scrollbar, transparent background
...
### Standalone
Open `scrollviewer.html` in a browser. Scroll to animate the image sequence.

### Embed in Website (iframe)
```html
<iframe 
  src="https://raw.githubusercontent.com/yourusername/scroll_animation/main/scrollviewer.html" 
  width="1080" 
  height="1920" 
  style="border: none;">
</iframe>
...
Adjust these in `scrollviewer.html` to customize behavior:
...
- `scrollviewer.html` - Main production viewer (1080x1920, no UI elements)
- `optimize_images.py` - Python script for batch converting PNG sequences to optimized JPEG
